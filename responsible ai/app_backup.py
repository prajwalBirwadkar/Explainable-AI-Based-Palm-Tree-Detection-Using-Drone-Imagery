...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown('<div class="sidebar-subheader">🌈 Display Options</div>', unsafe_allow_html=True)
        with st.container():
            st.markdown('<div style="padding: 10px; background-color: rgba(30, 136, 229, 0.1); border-radius: 5px; margin-bottom: 15px;">', unsafe_allow_html=True)
            color_scheme = st.selectbox(
                ...
                index=0,
                help="Choose color scheme for GradCAM heatmaps"
            )
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        # Use an image from the dataset_images folder as a sample
        sample_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dataset_images")
        
        if use_sample and os.path.exists(sample_directory):
            # Get the first image from the dataset_images directory
            ...
            with tab2:
                if len(filtered_boxes) > 0:
                    ...
                    display_option = st.radio(
                        ...
                        horizontal=True,
                        key="individual_analysis_display_option"
                    )
                    ...
                    if display_option == "Grid View":
                        ...
                        selected_detection = st.selectbox("Select a detection to analyze in detail:", detection_options)
                        ...
                        <div class="card" style="margin-top: 20px;">
                            <div class="card-title">Conclusion & Analysis</div>
                            <div style="padding: 10px;">
                                <p>Based on this detection's heatmap, the model appears to be focusing on 
                                {'the overall tree shape and fronds' if heatmap_data['score'] > 0.8 else 'specific features, but may need more certainty'}.</p>
                                <p>The detection confidence of {heatmap_data['score']:.2f} indicates 
                                {'very high certainty' if heatmap_data['score'] > 0.9 else 'good certainty' if heatmap_data['score'] > 0.7 else 'moderate certainty'}.</p>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
